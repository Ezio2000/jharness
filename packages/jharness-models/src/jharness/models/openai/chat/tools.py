...
def encode_tools(
    ...
    if RuntimeToolKind.STRUCTURED not in profile.capabilities.runtime_tool_kinds:
        raise OpenAIChatError(f"{profile.name} does not support tools")
    if any(not isinstance(tool, StructuredToolSpec) for tool in tools):
        raise OpenAIChatError(f"{profile.name} does not support freeform runtime tools")
...
def _decode_arguments(value: object) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise OpenAIChatError("chat completion tool function arguments are invalid JSON") from exc
    if not isinstance(parsed, Mapping):
        raise OpenAIChatError("chat completion tool function arguments must decode to an object")
