...
class OpenAIChatCodec:
    ...
    def _add_extra_request_body(self, payload: JsonObject) -> None:
        ...
        if collision:
            key = min(collision)
            raise OpenAIChatError(f"extra_request_body cannot set reserved request field: {key}")
    ...
    def decode_response(
        ...
        finish_reason = OPENAI_CHAT_JSON.required_string(
        ...
        return ModelResponse(
            ...
            model_id=OPENAI_CHAT_JSON.optional_string(value.get("model")),
            response_id=OPENAI_CHAT_JSON.optional_string(value.get("id")),
...
def _decode_message_reasoning(
    ...
    if not isinstance(value, str):
        raise OpenAIChatError("chat completion message reasoning_content must be a string or null")
...
def _response_metadata(value: Mapping[str, Any], provider: str) -> JsonObject:
    ...
    if object_value is not None:
        metadata["object"] = OPENAI_CHAT_JSON.required_string(
            object_value, "chat completion object"
        )
...
def decode_usage(value: object) -> ModelUsage | None:
    ...
    prompt_tokens = OPENAI_CHAT_JSON.optional_integer(usage.get("prompt_tokens"))
    completion_tokens = OPENAI_CHAT_JSON.optional_integer(usage.get("completion_tokens"))
    total_tokens = OPENAI_CHAT_JSON.optional_integer(usage.get("total_tokens"))
    ...
    if isinstance(completion_details, Mapping):
        ...
        reasoning_tokens = OPENAI_CHAT_JSON.optional_integer(
            completion_details_mapping.get("reasoning_tokens")
        )
    if isinstance(prompt_details, Mapping):
        prompt_details_mapping = cast(Mapping[str, object], prompt_details)
        cache_read_tokens = OPENAI_CHAT_JSON.optional_integer(
            prompt_details_mapping.get("cached_tokens")
        )
    if cache_read_tokens is None:
        cache_read_tokens = OPENAI_CHAT_JSON.optional_integer(usage.get("prompt_cache_hit_tokens"))
    return ModelUsage(
