...
from jharness.models.openai.responses_api.artifacts import (
...
class ResponsesProviderToolCodec(ABC):
    ...
    def stream_item_update(self, item: Mapping[str, Any]) -> ProviderStreamUpdate:
...
@dataclass(frozen=True, slots=True)
class ResponsesWebSearchTool(ResponsesProviderToolCodec):
    ...
    def stream_event_update(
        ...
        return ProviderStreamUpdate(status, data)


@dataclass(frozen=True, slots=True)
class ResponsesImageGenerationTool(ResponsesProviderToolCodec):
    ...
    def stream_event_update(
        ...
        return ProviderStreamUpdate(_provider_event_status(suffix, "image generation"))
...
@dataclass(frozen=True, slots=True)
class ResponsesProviderToolRegistry:
    """Immutable exact-identity registry consumed by the shared Responses codec."""

    codecs: tuple[ResponsesProviderToolCodec, ...] = ()
    _by_tool: Mapping[ProviderToolId, ResponsesProviderToolCodec] = field(
        init=False,
        repr=False,
        compare=False,
    )
    _by_output_item: Mapping[str, ResponsesProviderToolCodec] = field(
        init=False,
        repr=False,
        compare=False,
    )
    _event_codecs: tuple[ResponsesProviderToolCodec, ...] = field(
        init=False,
        repr=False,
        compare=False,
    )
    ...
    def __post_init__(self) -> None:
        ...
        by_tool: dict[ProviderToolId, ResponsesProviderToolCodec] = {}
        by_output: dict[str, ResponsesProviderToolCodec] = {}
        declarations: dict[str, ProviderToolId] = {}
        prefixes: dict[str, ProviderToolId] = {}
        codecs: list[ResponsesProviderToolCodec] = []
        for raw_codec in raw_values:
            if not isinstance(raw_codec, ResponsesProviderToolCodec):
                raise TypeError(
    ...
    def codec_for_tool(self, tool: ProviderToolId) -> ResponsesProviderToolCodec:
        try:
    ...
    def codec_for_output_item(self, item_type: str) -> ResponsesProviderToolCodec | None:
        return self._by_output_item.get(item_type)

    def codec_for_event(self, event_type: str) -> ResponsesProviderToolCodec | None:
        return next(
...
def _validate_codec_shape(codec: ResponsesProviderToolCodec) -> frozenset[str]:
