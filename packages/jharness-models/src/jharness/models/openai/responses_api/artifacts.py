...
@runtime_checkable
class ResponsesArtifactStore(Protocol):
...
async def hydrate_image_call(
    ...
    if not isinstance(raw_data, bytes):
        raise TypeError("ResponsesArtifactStore.load_image() must return bytes")
    data = raw_data
    _validate_artifact(artifact, data, media_type=media_type)
    return replace(
        call,
...
def _validate_artifact(
    ...
    *,
    ...
    if not isinstance(artifact, ArtifactRef):
        raise TypeError("ResponsesArtifactStore.save_image() must return ArtifactRef")
