...
class AnthropicServerToolCodec:
    ...
    def encode_declaration(self, spec: ProviderToolSpec) -> JsonObject:
        ...
        if unsupported:
            key = min(unsupported)
            raise AnthropicError(f"unsupported {self.tool.type} configuration field: {key}")
    ...
    def decode_call(
        ...
        if use is not None:
            if use.get("type") != "server_tool_use":
                raise AnthropicError("Anthropic server tool use requires type='server_tool_use'")
            call_name = ANTHROPIC_JSON.required_string(
                use.get("name"),
            ...
            if not isinstance(raw_input, Mapping):
                raise AnthropicError("Anthropic server tool input must be an object")
        ...
        if result is None:
            ...
            result_type = ANTHROPIC_JSON.required_string(
                result.get("type"),
    ...
    def encode_history(self, call: ProviderToolCall) -> list[JsonObject]:
        ...
        if native.get("server_tool_use") is True:
            ...
            if not isinstance(name, str) or name not in self.call_names:
                raise AnthropicError("Anthropic server tool history has an invalid call name")
            use_extra = native.get("use_extra", {})
            if not isinstance(use_extra, Mapping):
                raise AnthropicError("Anthropic server tool use metadata must be an object")
            blocks.append(
                {
        ...
        if call.output:
            ...
            if not isinstance(raw, Mapping):
                raise AnthropicError("Anthropic server tool result part requires native data")
            result = cast(
                JsonObject,
            ...
            if not isinstance(result_type, str) or result_type not in self.result_block_types:
                raise AnthropicError("Anthropic server tool history has an invalid result type")
        ...
        if not blocks:
            raise AnthropicError("Anthropic server tool history contains no native blocks")
        return blocks
    ...
    def _call_id(
        ...
        use_id = (
            None
            if use is None
            else ANTHROPIC_JSON.required_string(
                use.get("id"),
        ...
        if use_id is not None and result_id is not None and use_id != result_id:
            raise AnthropicError("Anthropic server tool result references a different use id")
        return use_id or cast(str, result_id)

    def _require_spec(self, spec: ProviderToolSpec) -> None:
        if spec.tool != self.tool:
            raise AnthropicError("Anthropic server tool spec has the wrong identity")

    def _require_call(self, call: ProviderToolCall) -> None:
        if call.tool != self.tool:
            raise AnthropicError("Anthropic server tool call has the wrong identity")


@dataclass(frozen=True, slots=True)
class AnthropicServerToolRegistry:
    """Immutable bidirectional registry for one Messages profile."""

    codecs: tuple[AnthropicServerToolCodec, ...] = ()
    _by_tool: Mapping[ProviderToolId, AnthropicServerToolCodec] = field(
        init=False,
        repr=False,
        compare=False,
    )
    _by_call_name: Mapping[str, AnthropicServerToolCodec] = field(
        init=False,
        repr=False,
        compare=False,
    )
    _by_result_type: Mapping[str, AnthropicServerToolCodec] = field(
        init=False,
        repr=False,
        compare=False,
    )
    ...
    def __post_init__(self) -> None:
        ...
        by_tool: dict[ProviderToolId, AnthropicServerToolCodec] = {}
        by_call_name: dict[str, AnthropicServerToolCodec] = {}
        by_result_type: dict[str, AnthropicServerToolCodec] = {}
        declaration_types: set[str] = set()
        validated: list[AnthropicServerToolCodec] = []
        for raw_codec in codecs:
            if not isinstance(raw_codec, AnthropicServerToolCodec):
    ...
    def codec_for_tool(self, tool: ProviderToolId) -> AnthropicServerToolCodec:
        try:
            return self._by_tool[tool]
        except KeyError as exc:
            raise AnthropicError(
                f"Anthropic profile does not support provider tool: {tool.namespace}/{tool.type}"
            ) from exc

    def codec_for_call_name(self, name: str) -> AnthropicServerToolCodec | None:
        return self._by_call_name.get(name)

    def codec_for_result_type(self, result_type: str) -> AnthropicServerToolCodec | None:
...
def anthropic_web_search_codec(
    tool: ProviderToolId,
    *,
...
def _native_metadata(call: ProviderToolCall) -> Mapping[str, Any]:
    ...
    if not isinstance(raw, Mapping):
        raise AnthropicError("Anthropic server tool history requires native metadata")
