...
def _decode_input(value: object) -> Mapping[str, Any]:
    ...
    if not isinstance(value, str):
        raise AnthropicMessagesError("Anthropic tool_use input must be an object or JSON string")
    try:
        parsed: object = json.loads(value)
    except json.JSONDecodeError as exc:
        raise AnthropicMessagesError("Anthropic tool_use input is invalid JSON") from exc
    if not isinstance(parsed, Mapping):
        raise AnthropicMessagesError("Anthropic tool_use input must decode to an object")
